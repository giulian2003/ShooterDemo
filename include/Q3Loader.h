 ...
 *
 * 01/01/2016: Modified by Iulian Ghetau to read data from a stream, 
 * convert to OpenGL coordinate system and post process the bezier patches.
 *
...
struct TEntity
{
  ...
  std::string  mBuffer;         ///< Entity descriptions, stored as a string. 
};
...
struct TNode
{
  ...
  float        mMins[3];        ///< Integer bounding box min coord (converted to float).  
  float        mMaxs[3];        ///< Integer bounding box max coord (converted to float).  
};
 ...
 */
typedef int TMeshVert;          ///< Vertex index offset, relative to first vertex of corresponding face.
...
struct TVisData
{
  ...
  std::vector<char> mBuffer;          ///< Array of bytes holding the cluster vis.
};
...
enum Q3MapPostProcessSteps 
{
  PostProcess_CoordSysOpenGL = 1,
  PostProcess_FlipWindingOrder = 1 << 1,
  PostProcess_IndexBezierPatches = 1 << 2,
  PostProcess_TriangulateBezierPatches = 1 << 3,
};
 ...
 */
bool readMap(std::istream& bspData, TMapQ3& pMap, float scale = 1.f, unsigned postProcessSteps = 0u);
