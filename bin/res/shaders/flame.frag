...
void main()
{
  vec2 uv = 1. - o_texCoordDiffuse;
  vec2 q = uv;
  //q.x *= 5.;
  q.y *= 2;
  float strength = floor(q.x+2.);
  float T3 = max(3.,1.25*strength) * u_globalTime;
  q.x = mod(q.x,1.)-0.5;
  q.y -= 0.25;
  float n = fbm(strength*q - vec2(0,T3));
  float pFlameW = 1.3; // affects the flame Width
  float pFlameH = .75; // affects the flame Height
  float c = 1. - 16. * pow( max( 0., length(q*vec2(pFlameW+q.y*1.5,pFlameH) ) - n * max( 0., q.y+.25 ) ),1.2 );
...
//  float c1 = n * c * (1.5-pow(2.50*uv.y,4.));
  ...
  resultingColor = vec4( mix(vec3(0.),col,a), a);
}
